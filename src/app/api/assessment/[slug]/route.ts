...
const schema = z.object({ answers: z.record(z.string(), answerValue) });

export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ slug: string }> }
) {
  const assessment = getExpansionAssessment((await params).slug);
  ...
  try {
    const report = buildExpansionReport(assessment, parsed.data.answers);
